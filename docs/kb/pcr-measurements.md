...
At the time of writing (June 2024) coreboot has no support for multiple PCR
banks but allows choosing a hash algorithm for TPM 2.0 log format.  This choice
is fixed at build time.

| TPM version | PCR Banks
| ----------- | --------
| 1.2         | SHA-1 only
| 2.0         | SHA-256 only
...
| ---   | ---------- | ---------  | -----------
| PCR-2 | EV_ACTION  | Always     | Stages, data, blobs, payload
...
| PCR-7    | EV_EFI_VARIABLE_DRIVER_CONFIG    | Always          | 61dfe48b-ca93-d211-aa0d-00e098032b8c:SecureBoot variable
|          |                                  |                 | 61dfe48b-ca93-d211-aa0d-00e098032b8c:PK variable
...
| PCR-7    | EV_EFI_VARIABLE_AUTHORITY        | Unknown         | Measurements done by a bootloader (GRUB, shim, etc.), listed here to show where they appear
...
affecting event log in multiple places, for example: disabling a device will
change value of a corresponding option and possibly remove an entry with a
measurement of its OptionROM.
