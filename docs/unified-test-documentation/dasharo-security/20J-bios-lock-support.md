...
## BLS003.001 BIOS lock support deactivation (Ubuntu 22.04)

**Test description**

This test aims to verify that BIOS lock can be disabled and is no logner
blocking changes in BIOS region of flash chip.
...
1. Enter the `Dasharo System Features` menu using the arrow keys and Enter.
1. Enter the `Dasharo Security Options` submenu.
1. Verify that the `Lock the BIOS boot medium` option is disabled - if not,
    press `Enter` and then `F10` to save the changes.
1. If necessary - press `Y` to confirm saving the changes.
1. Go back to the main menu using the `ESC` key.
1. Select the `Reset` option to apply the settings and reboot.
